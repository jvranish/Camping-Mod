...
public class ContainerCampfireCheapCooker extends Container
{
    private TileEntityCampfireCheapCooker campfire;
    private int lastCookTime = 0;
    private int lastBurnTime = 0;
    private int lastItemBurnTime = 0;

    public ContainerCampfireCheapCooker(InventoryPlayer par1InventoryPlayer, TileEntityCampfireCheapCooker par2TileEntitycampfire)
    {
        this.campfire = par2TileEntitycampfire;
        this.addSlotToContainer(new Slot(par2TileEntitycampfire, 0, 43, 28));
        this.addSlotToContainer(new Slot(par2TileEntitycampfire, 1, 43, 49));
        this.addSlotToContainer(new SlotFurnace(par1InventoryPlayer.player, par2TileEntitycampfire, 2, 116, 28));
        this.addSlotToContainer(new SlotFurnace(par1InventoryPlayer.player, par2TileEntitycampfire, 3, 116, 49));
    ...
    public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2)
    {
        ...
        if (var4 != null && var4.getHasStack())
        {
            ...
            if (par2 == 2||par2 == 3)
            {
                if (!this.mergeItemStack(var5, 5, 40, true))
                {
                    return null;
                }
            ...
            else if (par2 != 1 && par2 != 0 && par2!=4)
            {
                ...
                {
                    if (!this.mergeItemStack(var5, 32, 40, false))
                    {
                        return null;
                    }
                }
                else if (par2 >= 32 && par2 < 40 && !this.mergeItemStack(var5, 5, 32, false))
                {
                    return null;
                }
            }
            else if (!this.mergeItemStack(var5, 5, 40, false))
            {
                return null;
            }
